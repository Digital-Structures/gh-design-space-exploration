...
namespace StructureEngine.Model
{
    public class Structure : BaseDesign
    {
        ...
        private void InitializeLists(List<Node> nodes, List<Member> members) // changed from private to public
        {
            if (nodes == null)
            {
        ...
        public override List<IVariable> DesignVariables
        {
            get
            {
                ...
                foreach (IVariable var in this.DOFs)
                {
                    if (var.Free)
                    {
                        dv.Add(var);
        ...
        private List<DOF> AffectedDOFs
        {
            get
            {
                ...
                foreach (DOF d in this.DOFs)
                {
                    ...
                    {
                        ad.Add(d);
                        foreach (ParametricRelation r in d.Relations)
                        {
                            foreach (Node n in r.Listeners)
                            {
                                if (d.IsX)
                                {
                                    ad.Add(n.DOFs[0]);
            ...
            foreach (Node nd in Nodes)
            {
                for (int j = 0; j < nd.DOFs.Length; j++)
                {
                    ...
                    foreach (ParametricRelation p in d.Relations)
                    {
                        ...
                        nodeMap[nd].DOFs[j].Relations.Add(newp);
                    }
                }
            }
